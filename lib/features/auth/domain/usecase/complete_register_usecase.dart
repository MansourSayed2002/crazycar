  ...
  Future<ApiResult> completeRegister(List<File> image) async {
    try {
      ...
      await authRepoAbs.completeRegister({
        "driver_idcard":
            "${TablesDatabaseApp.imagePathCompleteDiver}/${user.userId}/${image[0].toString().split("/").last}",
        "driver_idcardback":
            "${TablesDatabaseApp.imagePathCompleteDiver}/${user.userId}/${image[1].toString().split("/").last}",
        "driver_you_idcard":
            "${TablesDatabaseApp.imagePathCompleteDiver}/${user.userId}/${image[2].toString().split("/").last}",
  ...
  uploadImageUser(List<File> image, TypeUser type) async {
    ...
    for (var i = 0; i < image.length; i++) {
      await authRepoAbs.uplaodImageUser(
        image[i],
        "${TablesDatabaseApp.imagePathCompleteDiver}/${user.userId}/${image[i].toString().split("/").last}",
        user.userPhone.toString(),
        type,
      );
    }
